...
_HIHATS = [
    ...
    _cfg('hihat', 'hihat', -17, '16n', 'NoiseSynth',
         ...
         ['C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N,'C3',_N]),
]

_BASSES = [
    # Dark A Phrygian chromatic bass — filter sweep adds movement
    _cfg('bass', 'bass', -4, '16n', 'FMSynth',
         ...
         automation=[_auto('effect:1:frequency', 500, 2400, 'tri')]),
    ...
    # E Phrygian alien bass — filter sweep + reverb tail
    _cfg('bass', 'bass', -4, '16n', 'AMSynth',
         ...
         automation=[_auto('effect:1:frequency', 400, 2200, 'tri')]),
    # D minor heavy sub
...
_LEADS = [
    ...
    _cfg('lead', 'lead', -10, '16n', 'AMSynth',
         ...
         ['B4',_N,'C5',_N,'B4',_N,'A4',_N,'G4','A4','B4',_N,'C5',_N,'B4',_N,'B4','A4',_N,'G4','A4','B4',_N,'D5','C5','B4',_N,'A4',_N,'G4',_N,_N]),
    _cfg('lead', 'lead', -12, '16n', 'FMSynth',
         ...
         ['D5',_N,'Eb5','D5',_N,'C5',_N,'D5','Eb5',_N,'D5','C5','Bb4',_N,'C5','D5','D5',_N,'Eb5',_N,'F5','Eb5','D5',_N,'C5','D5',_N,'C5','Bb4','C5','D5',_N]),
]
...
class ForestPsyTrackGenerator(BaseTrackGenerator):
    ...
    def _generate_layers(cls) -> list[dict[str, Any]]:
        ...
        bass['pattern']['velocities'] = _vel(bass['pattern']['steps'], accent_prob=0.25, ghost_prob=0.1)
        hihat['pattern']['velocities'] = _vel_groove(hihat['pattern']['steps'])
        lead['pattern']['velocities'] = _vel(lead['pattern']['steps'], accent_prob=0.22, ghost_prob=0.08)

        # Lead with optional delayed entry — adds breathing space at track start
        if random.random() < 0.4:
            lead['entry_loop'] = random.choice([2, 4])

        layers = [kick, bass, hihat, lead]

        # Atmospheric texture in ~half of tracks
        if random.random() < 0.5:
            texture = copy.deepcopy(cls._pick(_TEXTURES))
