...
class UnshareItineraryListSchema(Schema):
    identifier: str


class UnshareItineraryListView(RunActionOnItemByIdAPIView):
    @classmethod
    ...
    def get_data_schema(cls) -> Type[Schema]:
        return UnshareItineraryListSchema
    ...
    @classmethod
    async def run_action(cls, request: APIRequest, obj: ItineraryList, data: UnshareItineraryListSchema, path: Path) -> None:
        user = await request.future_user
        await ItineraryListManager(user).unshare(obj, data.identifier)
        return None
