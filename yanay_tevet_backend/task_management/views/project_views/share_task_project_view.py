...
class ShareTaskProjectSchema(Schema):
    identifier: str
    role: ProjectRole = ProjectRole.COLLABORATOR


class ShareTaskProjectView(RunActionOnItemByIdAPIView):
    @classmethod
    ...
    @classmethod
    async def run_action(cls, request: APIRequest, obj: TaskProject, data: ShareTaskProjectSchema, path: Path) -> None:
        user = await request.future_user
        await TaskProjectManager(user).share(obj, data.identifier, data.role)
        return None
