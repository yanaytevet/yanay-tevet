...
from apartment_hunt.managers.villa_villekulla_manager import VillaVillekullaManager
...
class InviteToVillaVillekullaSchema(Schema):
    identifier: str
    role: ProjectRole = ProjectRole.COLLABORATOR


class ShareVillaVillekullaProjectView(RunActionOnItemByIdAPIView):
    @classmethod
    ...
    def get_data_schema(cls) -> Type[Schema]:
        return InviteToVillaVillekullaSchema
    ...
    @classmethod
    async def run_action(cls, request: APIRequest, obj: RentalProject, data: InviteToVillaVillekullaSchema, path: Path) -> None:
        user = await request.future_user
        await VillaVillekullaManager(user).invite(obj, data.identifier, data.role)
        return None
