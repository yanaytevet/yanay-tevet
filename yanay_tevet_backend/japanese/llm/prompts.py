...
_OUTPUT_CONTRACT = (
    ...
    "the identifying ones). The discriminator (data.node_type) MUST equal the node's type. If the "
    "input identity is wrong or non-canonical (e.g. a word given as なか when it should be 中, a "
    "kanji handed to you incorrectly, a particle with whitespace), correct it in `data` — the "
    "node's canonical_key is re-derived from your `data` after generation, and the node will be "
    "renamed or merged into the existing canonical node automatically. Always emit the canonical "
    "form rather than preserving a wrong input.\n"
