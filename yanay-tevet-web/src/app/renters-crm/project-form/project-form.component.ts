...
import {CURRENCY_LABELS, CURRENCY_ORDER} from '../renters-crm.constants';
...
export class RentersProjectFormComponent {
  ...
  readonly initialAskedRentCtrl = new FormControl<number | null>(null);
  ...
  private async loadProject(id: number): Promise<void> {
    ...
    try {
      ...
      this.initialAskedRentCtrl.setValue(res.data.initial_asked_rent);
    } catch {
  ...
  async onSave(): Promise<void> {
    ...
    try {
      const body = {
        ...
        initial_asked_rent: this.initialAskedRentCtrl.value,
      };
      const id = this.projectId();
      if (id !== null) {
        await updateRentalProjectView({body, path: {object_id: id}});
        await this.routingService.navigateToRentersCrmProject(id);
      } else {
        const res = await createRentersProjectView({body});
        await this.routingService.navigateToRentersCrmProject(res.data.id);
      }
    } catch (err) {
